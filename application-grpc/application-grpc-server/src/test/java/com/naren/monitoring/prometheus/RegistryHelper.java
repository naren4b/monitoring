...
public class RegistryHelper {
	public static Optional<Collector.MetricFamilySamples> findRecordedMetric(String name,
	...
	public static Collector.MetricFamilySamples findRecordedMetricOrThrow(String name,
			CollectorRegistry collectorRegistry) {
	...
	public static double extractMetricValue(String name, CollectorRegistry collectorRegistry) {
		...
		if (result.samples.size() != 1) {
			throw new IllegalArgumentException(
					String.format("Expected one value, but got %d for metric %s", result.samples.size(), name));
		}
		return result.samples.get(0).value;
	}

	public static int countSamples(String metricName, String sampleName, CollectorRegistry collectorRegistry) {
		Enumeration<Collector.MetricFamilySamples> samples = collectorRegistry.metricFamilySamples();
		while (samples.hasMoreElements()) {
			Collector.MetricFamilySamples sample = samples.nextElement();
